...
for index in range(len(best_model_list)):
    ...
    # Define the scope name used in your model definition
    model_scope = 'DeepCas9'
    with tf.Session(config=conf) as sess:
        ...
        # Filter variables by the model's scope
        model_variables = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=model_scope)
        weights = {v.name: sess.run(v) for v in model_variables}
        # Save weights in .h5 file
        with h5py.File('/home/yc774/ondemand/OOD/cellinfinity/Paired-Library/DeepCas9/DeepCas9_Final/model.weights.h5', 'w') as hf:
